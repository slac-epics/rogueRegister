    ...
    while (std::getline(fStream, line, delim)) {
        ...
        while (std::getline(lStream, field, '\t')) {
            if (doKey)
                ...
                fields.insert(std::pair<std::string, std::string>(key[x], field));
            ++x;
        }
...
void rogue::LibraryBase::createVariable(std::map<std::string, std::string>& data,
    ...
    // Create variable
    rim::VariablePtr var = rim::Variable::create(name,
                                                 mode,
                                                 minimum,
                                                 maximum,
                                                 offset,
...
std::string rogue::LibraryBase::getFieldString(std::map<std::string, std::string> fields, std::string name) {
    if (fields.find(name) == fields.end())
        throw(rogue::GeneralError::create("LibraryBase::getFieldString",
                                          "Failed to find field '%s' in exported address map",
                                          name.c_str()));
...
uint64_t rogue::LibraryBase::getFieldUInt64(std::map<std::string, std::string> fields, std::string name) {
    ...
    if (fields.find(name) == fields.end())
        throw(rogue::GeneralError::create("LibraryBase::getFieldUInt64",
                                          "Failed to find field '%s' in exported address map",
                                          name.c_str()));
    ...
    try {
        ...
        throw(rogue::GeneralError::create("LibraryBase::getFieldUInt64",
                                          "Failed to extract uint64_t from field '%s' in exported address map",
                                          name.c_str()));
    }
    return ret;
}
...
uint32_t rogue::LibraryBase::getFieldUInt32(std::map<std::string, std::string> fields, std::string name, bool def) {
    ...
    if (fields.find(name) == fields.end()) {
        if (def)
            ...
            throw(rogue::GeneralError::create("LibraryBase::getFieldUInt32",
                                              "Failed to find field '%s' in exported address map",
                                              name.c_str()));
    }
    ...
    try {
        ...
        throw(rogue::GeneralError::create("LibraryBase::getFieldUInt32",
                                          "Failed to extract uint32_t from field '%s' in exported address map",
                                          name.c_str()));
    }
    return ret;
}
...
bool rogue::LibraryBase::getFieldBool(std::map<std::string, std::string> fields, std::string name) {
    if (fields.find(name) == fields.end())
        throw(rogue::GeneralError::create("LibraryBase::getFieldBool",
                                          "Failed to find field '%s' in exported address map",
                                          name.c_str()));
...
double rogue::LibraryBase::getFieldDouble(std::map<std::string, std::string> fields, std::string name) {
    ...
    if (fields.find(name) == fields.end())
        throw(rogue::GeneralError::create("LibraryBase::getFieldDouble",
                                          "Failed to find field '%s' in exported address map",
                                          name.c_str()));
    ...
    try {
        ...
        throw(rogue::GeneralError::create("LibraryBase::getFieldDouble",
                                          "Failed to extract double from field '%s' in exported address map",
                                          name.c_str()));
    }
    return ret;
}
...
std::vector<uint32_t> rogue::LibraryBase::getFieldVectorUInt32(std::map<std::string, std::string> fields,
                                                               std::string name) {
    ...
    if (fields.find(name) == fields.end())
        throw(rogue::GeneralError::create("LibraryBase::getFieldVector32",
                                          "Failed to find field '%s' in exported address map",
                                          name.c_str()));
    ...
    while (std::getline(fStream, sub, ',')) {
        try {
            ...
            throw(rogue::GeneralError::create("LibraryBase::getFieldVectorUInt32",
                                              "Failed to extract uint32 from field '%s' in exported address map",
                                              name.c_str()));
        }
        ret.push_back(val);
    }
